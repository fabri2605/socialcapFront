...
class MemoryStore {
    ...
    prepareUpdateRoot(root) {
        this.operationCache.push({
            opType: 0 /* OperationType.put */,
            setType: 0 /* SetType.nodes */,
            k: 'root',
            v: [root],
        });
    }
    ...
    async commit() {
        for (let i = 0, len = this.operationCache.length; i < len; i++) {
            ...
            if (v.opType === 0 /* OperationType.put */) {
                if (v.setType === 0 /* SetType.nodes */) {
