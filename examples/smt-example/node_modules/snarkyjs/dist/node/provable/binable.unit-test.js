...
import { BinableString, BinableUint32, BinableUint64, defineBinable, record, withCheck, withVersionNumber, } from './binable.js';
import { PublicKey, Scalar } from './curve-bigint.js';
import { Bool, Field, UInt64 } from './field-bigint.js';
// uint
...
test.negative(Random.publicKey.invalid, (publicKey) => PublicKey.fromBytes(PublicKey.toBytes(publicKey)));
// string
...
// should fail to decode when check fails
Bool.fromBytes(Bool.toBytes(bool)));
let randomAccountBalance = Random.record({
    address: Random.publicKey,
    balance: Random.uint64,
});
let AccountBalance = record({ address: PublicKey, balance: UInt64 }, ['address', 'balance']);
...
expect(() => Field.readBytes(fieldBytes, -1)).toThrow();
expect(() => Field.readBytes(fieldBytes, 0.1)).toThrow();
expect(() => Field.readBytes(fieldBytes, 3e-9)).toThrow();
expect(() => Field.readBytes(fieldBytes, fieldBytes.length)).toThrow();
//# sourceMappingURL=binable.unit-test.js.map