...
import { Bool, Field, Ledger, Pickles, Poseidon as Poseidon_, } from '../snarky.js';
import { AccountUpdate, Authorization, Body, CallForest, Events, Permissions, Actions, signJsonTransaction, smartContractContext, TokenId, ZkappCommand, zkAppProver, ZkappPublicInput, ZkappStateLength, } from './account_update.js';
import { Circuit, circuitArray, cloneCircuitValue, getBlindingValue, memoizationContext, provable, Struct, toConstant, } from './circuit_value.js';
...
import { analyzeMethod, compileProgram, emptyValue, GenericArgument, getPreviousProofsForProver, inAnalyze, inCompile, inProver, isAsFields, methodArgumentsToConstant, methodArgumentTypesAndValues, Proof, snarkContext, sortMethodArguments, } from './proof_system.js';
import { PrivateKey, PublicKey } from './signature.js';
import { assertStatePrecondition, cleanStatePrecondition } from './state.js';
// external API
export { SmartContract, method, signFeePayer, declareMethods, Callback, Account, VerificationKey, Reducer, };
    ...
    return function wrappedMethod(...actualArgs) {
            ...
            return smartContractContext.runWith(smartContractContext() ?? {
                ...
                if ((inCompile() || inProver() || inAnalyze()) &&
                    ...
                    let [{ memoized }, result] = memoizationContext.runWith({
                        ...
                        currentIndex: 0,
                        blindingValue,
                    }, () => method.apply(this, actualArgs.map((a, i) => {
        ...
        let [, result] = smartContractContext.runWith({
            ...
            let isTopLevel = Circuit.witness(Bool, () => {
                ...
                let isProverUpdate = inProver() &&
                    zkAppProver
                        .getData()
                        .accountUpdate.body.publicKey.equals(parentAccountUpdate.body.publicKey)
                        .toBoolean();
...
class SmartContract {
    ...
    static async compile() {
        ...
        this.analyzeMethods();
        let { getVerificationKeyArtifact, provers, verify } = compileProgram(ZkappPublicInput, methodIntfs, methods, this);
...
Use the optional \`maxTransactionsWithActions\` argument to increase this number.`);
        ...
        getActions({ fromActionState, endActionState, }) {
            ...
            Circuit.asProver(() => {
                let actions = Mina.getActions(contract.address, { fromActionState, endActionState }, contract.self.tokenId);
        ...
        async fetchActions({ fromActionState, endActionState, }) {
            let result = await Mina.fetchActions(contract.address, { fromActionState, endActionState }, contract.self.tokenId);
            if ('error' in result) {
