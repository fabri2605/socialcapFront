...
// internal API
export { cloneCircuitValue, circuitValueEquals, circuitArray, memoizationContext, memoizeWitness, getBlindingValue, toConstant, isConstant, HashInput, };
const HashInput = {
    get empty() {
        return {};
    },
...
function circuitMain(target, propertyName, _descriptor) {
    ...
    target.snarkyWitnessTyp = provableFromTuple(Array.from(witnessIndexSet).map((i) => paramTypes[i]));
    target.snarkyPublicTyp = provableFromTuple(Array.from(publicIndexSet).map((i) => paramTypes[i]));
}
...
Circuit.constraintSystemFromKeypair = function (keypair) {
    return gatesFromJson(JSON.parse(keypair._constraintSystemJSON()))
        .gates;
};
