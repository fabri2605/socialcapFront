...
for (let [G, Field, Scalar] of [
    ...
    test(randomPoint, randomPoint, randomPoint, randomScalar, randomScalar, randomField, (X, Y, Z, x, y, f) => {
        // check on curve
        ...
        assert(!G.isOnCurve({ x: Field.power(f, 2n), y: Field.power(f, 3n), z: 1n }), 'x^3 = y^2 is not on curve');
        // equal
        assert(G.equal(X, X), 'equal');
        assert(!G.equal(X, G.add(X, X)) || G.equal(X, G.zero), 'not equal to double of itself (or zero)');
        assert(!G.equal(X, G.negate(X)) || G.equal(X, G.zero), 'not equal to negation of itself (or zero)');
        ...
        assert(G.equal(G.scale(X, Scalar.add(x, y)), G.add(G.scale(X, x), G.scale(X, y))), 'distributive');
        assert(G.equal(G.scale(X, Scalar.negate(x)), G.negate(G.scale(X, x))), 'distributive (negation)');
        assert(G.equal(G.scale(X, Scalar.mul(x, y)), G.scale(G.scale(X, x), y)), 'scale / multiply is associative');
        // endomorphism
        ...
        assert(G.equal(X, G.zero) ||
            Field.square(ya) === Field.add(Field.power(xa, 3n), G.b), 'affine on curve (or zero)');
    });
}
//# sourceMappingURL=elliptic_curve.unit-test.js.map